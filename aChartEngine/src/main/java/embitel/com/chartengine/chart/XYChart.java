 ...
 */
package embitel.com.chartengine.chart;
...
import embitel.com.chartengine.model.Point;
import embitel.com.chartengine.model.SeriesSelection;
import embitel.com.chartengine.model.XYMultipleSeriesDataset;
import embitel.com.chartengine.model.XYSeries;
import embitel.com.chartengine.renderer.BasicStroke;
import embitel.com.chartengine.renderer.DefaultRenderer;
import embitel.com.chartengine.renderer.SimpleSeriesRenderer;
import embitel.com.chartengine.renderer.UserYGrid;
import embitel.com.chartengine.renderer.XYMultipleSeriesRenderer;
import embitel.com.chartengine.renderer.XYMultipleSeriesRenderer.Orientation;
import embitel.com.chartengine.renderer.XYSeriesRenderer;
import embitel.com.chartengine.util.Helper;
import embitel.com.chartengine.util.MathHelper;
...
public abstract class XYChart extends AbstractChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        if (showGridX || showGridY) {
            ...
            drawYLabels(allYLabels, canvas, paint, maxScaleNumber, left, right, bottom, yPixelsPerUnit,
                    minY);
         /*   // @TODO <JC> TBC to integrate to new release
            drawYUserLabels(canvas, paint,
                    maxScaleNumber, left, right, bottom, yPixelsPerUnit, minY, maxY);
            // </JC>*/
     ...
     */
    void drawYLabels(Map<Integer, List<Double>> allYLabels, Canvas canvas, Paint paint,
                     int maxScaleNumber, int left, int right, int bottom, double[] yPixelsPerUnit, double[] minY) {
        Orientation or = mRenderer.getOrientation();
        boolean showGridX = mRenderer.isShowGridX();
        if (showGridX) {
            mGridPaint.setStyle(Style.STROKE);
            if (mRenderer.isXPathEffect()) {
                mGridPaint.setPathEffect(new DashPathEffect(new float[]{10, 10}, 0));
                mGridPaint.setStrokeWidth(mRenderer.getGridLineWidth() + 4);
            } else {
                mGridPaint.setStrokeWidth(mRenderer.getGridLineWidth());
            }
        }
        ...
        // clear path effect
        mGridPaint.setPathEffect(new DashPathEffect(new float[]{0, 0}, 0));
    }

    //@TODO <JC> TBC to integrate to new release ? build on 0.7.0 ?

    protected void drawYUserLabels(Canvas canvas, Paint paint, int maxScaleNumber, int left,
                                   int right, int bottom, double[] yPixelsPerUnit, double[] minY, double[] maxY) {

        boolean showUserYGrid = mRenderer.isShowUserYGrid();
        List<UserYGrid> userYGrid = mRenderer.getUserYGrid();

        if (showUserYGrid && userYGrid != null) {
            ...
            for (int i = 0; i < maxScaleNumber; i++) {
                ...
                // select the grid in the display area first
                List<UserYGrid> relevantUserYGrid = new ArrayList<UserYGrid>();
                for (Iterator<UserYGrid> iterator = userYGrid.iterator(); iterator.hasNext(); ) {
                    UserYGrid grid = (UserYGrid) iterator.next();
                    if (grid.getYValue() > minY[i] && grid.getYValue() < maxY[i])
                        relevantUserYGrid.add(grid);
                }

                paint.setTextAlign(mRenderer.getYLabelsAlign(i));
                int length = relevantUserYGrid.size();

                for (int jj = 0; jj < length; jj++) {
                    UserYGrid grid = (UserYGrid) relevantUserYGrid.get(jj);
                    String label = grid.getTitle();
                    Align axisAlign = mRenderer.getYAxisAlign(i);
                    boolean isTextLabel = (label != null);
                    float yLabel = (float) (bottom - yPixelsPerUnit[i] * (grid.getYValue() - minY[i]));

                    BasicStroke stroke = grid.getStroke();
                    Cap cap = paint.getStrokeCap();
                    Join join = paint.getStrokeJoin();
                    float miter = paint.getStrokeMiter();
                    PathEffect pathEffect = paint.getPathEffect();
                    ...
                    paint.setColor(grid.getGridColor()); // always use the color for text and line
                    ...
                    if (or == Orientation.HORIZONTAL) {
                        ...
                        if (isTextLabel) { // is it a bug ... on the original
                            canvas.drawLine(right - getLabelLinePos(axisAlign), yLabel, right, yLabel, paint);
...
// </JC>
     ...
     * @param maxX                the maximum value on the X axis in the chart
     */
    void drawXTextLabels(Double[] xTextLabelLocations, Canvas canvas, Paint paint,
